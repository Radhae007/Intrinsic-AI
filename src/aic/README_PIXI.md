...
This ROS kilted workspace is configured to use [Pixi](https://pixi.sh) for dependency management.
...
1. **Use a custom channel** — re-run init with `--channel` if the package lives elsewhere:
   ```bash
   pixi ros init --distro kilted --channel https://prefix.dev/my-channel
   ```
...
- [ROS kilted documentation](https://docs.ros.org/en/kilted/)
